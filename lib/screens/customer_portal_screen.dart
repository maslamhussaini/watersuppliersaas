...
class WsCustomerPortalScreen extends StatefulWidget {
  ...
  @override
  State<WsCustomerPortalScreen> createState() => _WsCustomerPortalScreenState();
}
...
class _WsCustomerPortalScreenState extends State<WsCustomerPortalScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final dels = await WsDataService.fetchDeliveries(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        // The notifications bell here did nothing — removed rather than left
              ...
              child: ListView(
                ...
                children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  '${c.areaName ?? '—'} · Rs ${c.effectiveRate.toStringAsFixed(0)}/bottle',
                                  style: const TextStyle(
                                ...
                                WsBadge(
                                  label: due > 0
                                      ? 'Rs ${_money.format(due)} Due'
                                      : 'Settled',
                                  bg: due > 0
                                      ? WsColors.redLight.withValues(alpha: 0.5)
                                      : WsColors.greenLight,
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: WsKpiCard(
                            icon: '💧',
                            value: '${c.bottleBalance}',
                            label: 'Filled Bottles With Me',
                            trend: 'Tap for history',
                            accentColor: WsColors.teal,
                            onTap: () => _showDetail(
                              '💧 Delivery History',
                              _DeliveryHistoryDetail(
                        ...
                        Expanded(
                          child: WsKpiCard(
                            icon: '📦',
                            value:
                                '${c.bottleBalance > 0 ? c.bottleBalance : 0}',
                            label: 'Empty Bottles To Return',
                            trend: 'Please return',
                            accentColor: WsColors.amber,
                            onTap: () => _showDetail(
                              '📦 Bottles Status',
                  ...
                  _buildFullWidthCard(
                    ...
                    label:
                        'Payments Made - Rs ${_money.format(totalPaid)} Total',
                  ...
                  ..._deliveries
                      ...
                      .map(
                        (d) => ListTile(
                          ...
                          title: Text(
                            DateFormat('d MMMM yyyy').format(d.deliveryDate),
                            style: const TextStyle(
                          ...
                          trailing: Text(
                            'Rs ${_money.format(d.amountCharged)}',
                            style: const TextStyle(
                              fontWeight: FontWeight.w700,
                              color: WsColors.red,
                              fontSize: 16,
    ...
    children: deliveries
        .map(
          (d) => ListTile(
            ...
            subtitle: Text(
              '+${d.bottlesDelivered} del · ${d.bottlesReturned} ret · Bal: ${d.bottleBalance}',
            ),
            trailing: Text(
              'Rs ${money.format(d.amountCharged)}',
              style: const TextStyle(
                fontWeight: FontWeight.w600,
                color: WsColors.primary,
              ),
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ListTile(
        dense: true,
        leading: const Text('💧'),
        title: const Text('Filled bottles with you'),
      ...
      if (balance > 0)
        Container(
          ...
          decoration: BoxDecoration(
            color: WsColors.amberLight,
            borderRadius: BorderRadius.circular(10),
          ),
          child: const Text(
            'Please return empty bottles on your next delivery visit.',
...
class _BalanceDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    children: [
      Center(
        child: Column(
          children: [
            ...
            Text(
              totalDue > 0 ? 'Outstanding due' : 'Fully settled',
      ...
      ...deliveries
          .where((d) => d.amountCharged > 0)
          .map(
            (d) => ListTile(
              dense: true,
              title: Text(DateFormat('dd MMM').format(d.deliveryDate)),
              subtitle: Text(
                '${d.bottlesDelivered} bottles × Rs ${d.rateApplied.toStringAsFixed(0)}',
              ),
              trailing: Text(
                'Rs ${money.format(d.amountCharged)}',
                style: const TextStyle(
    ...
    children: [
      ...payments.map(
        (p) => ListTile(
          ...
          trailing: Text(
            '+Rs ${money.format(p.amountReceived)}',
            style: const TextStyle(
              fontWeight: FontWeight.w600,
