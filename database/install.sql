...
--
-- Use this instead of running migrations/000..019 by hand.
--
...
-- =============================================================================
-- ## SECTION 019 — 019_plan_limits.sql
-- =============================================================================
--
-- ADDED FOR LAUNCH BLOCKER B2. This file previously stopped at 018, so a clean
-- install produced a schema with NO free-tier customer cap: ws_tblplans
-- .maxcustomers was a number the account screen displayed and nothing enforced.
-- The two-step workaround (run install.sql, then 019 by hand) depended on
-- somebody remembering, and the header above still said "000..013", so the file
-- actively misdescribed what it covered.
--
-- Re-runnable: the function is `create or replace`, and both triggers are
-- dropped with `if exists` before being created.
--
-- =============================================================================
...
-- VERIFICATION — 019
--
--   -- 1. the function exists, in the ws schema
--   select 1 from pg_proc p
--     join pg_namespace n on n.oid = p.pronamespace
--    where n.nspname = 'ws' and p.proname = 'tg_customer_plan_limit';
--   -- must return exactly 1 row
--
--   -- 2. BOTH triggers exist on ws_tblcustomers and are enabled
--   select tgname, tgenabled
--     from pg_trigger
--    where tgrelid = 'public.ws_tblcustomers'::regclass
--      and tgname in ('trg_customer_plan_limit_ins',
--                     'trg_customer_plan_limit_upd')
--      and not tgisinternal;
--   -- must return 2 rows, both with tgenabled = 'O'
--
--   -- 3. the cap actually refuses. As a free organization already at
--   --    maxcustomers, this must raise P0001 rather than inserting:
--   --      insert into public.ws_tblcustomers (orgid, customername, areaid,
--   --                                          isactive)
--   --      values (<orgid>, 'over the cap', <areaid>, true);
--
--   -- The full behavioural check — boundary, direct-insert bypass, CSV import,
--   -- same-clientuuid retry at the cap, soft-delete-then-create, reactivation,
--   -- grandfathered over-limit orgs, unlimited pro, past_due, concurrent
--   -- inserts and tenant isolation — is:
--   --      dart run bin/plan_limits.dart      (test_harness)
-- =============================================================================
